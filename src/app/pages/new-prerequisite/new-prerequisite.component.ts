...
export class NewPrerequisiteComponent implements OnInit, OnDestroy {
  ...
  getManagers() {
    ...
    this.http.getReq('/getAllManagers', { headers }).subscribe(res => {
      this.managers = res;
    }, err => {
      this.managers = [];
    });
  ...
  onSubmit() {
    ...
    this.http.postReq(`/prerequisites/add/${this.subDomainId}`, this.prerequisiteFrom.value, {headers}).subscribe(res => {
